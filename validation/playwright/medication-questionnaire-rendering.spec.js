...
defineTaskRenderingTest('medication-questionnaire', {
  ...
  extraChecks: async (page) => {
    ...
    // The plugin picks its controls from the device (plugin-question-screen.js
    ...
    const mode = await screen.evaluate((el) => ({
      touch: el.classList.contains('qsc-touch'),
      keyboard: el.classList.contains('qsc-keyboard'),
    }));
    expect(
    ...
    const keypadCount = await page.locator('.qsc-keypad').count();
    const typedFieldCount = await page.locator('#qsc-number').count();
    if (mode.keyboard) {
    ...
    // The slide transition is driven by --qsc-transition, set per trial from
