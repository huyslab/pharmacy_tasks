...
test('an unrecognized session label stops the run before any task loads', async ({ page }) => {
  ...
  await page.goto('/experiment.html?participant_id=session-check&context=relmed&module=pilot&session=Visit%203');
...
test('the resolved session reaches every task in a module', async ({ page }) => {
  ...
  await page.goto('/experiment.html?participant_id=session-check&context=relmed&module=pilot&session=Session%202');
